...
export const Market = () => {
    ...
    const availableSnacks = useMemo<Snack[]>(() => PossibleBuys(credits),[credits]);
    ...
    const renderBasket = (<Card>
        <Card.Header>Basket {credits}</Card.Header>
        {basket.map((snack:Snack, index: number) => <Button key={`basket-snack-${index}-${snack.name}`} onClick={() => handleSellSnack(snack, index)}>{snack.name}</Button> )}
    </Card>)

    const renderPossibleSnacks = (<Card>
        <Card.Header>Market</Card.Header>
        {availableSnacks.map((snack:Snack) => <Button key={`possible-buys-${snack.name}`} onClick={() => handleBuySnack(snack)}>{snack.name}</Button> )}
    </Card>
)
